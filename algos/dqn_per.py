...
if __name__ == "__main__":
    ...
    with logger.RewardTracker(writer, params['stop_reward']) as reward_tracker:
        while True:
            ...
            loss_v, sample_priorities_v = loss.calc_weighted_loss_dqn(batch, batch_weights, net,
                                                    tgt_net.target_model, params['gamma'], device=device)

            loss_v.backward()
            optimizer.step()

            buffer.update_priorities(batch_indices, sample_priorities_v.data.cpu().numpy())
